<template>
  <div class="buffer-form">
    <div class="form-item">
      <label for="buffer-layer">图层选择</label>
      <select id="buffer-layer" v-model="selectedLayerId" class="custom-input">
        <option disabled value="">选择图层</option>
        <option v-for="layer in availableLayers" :key="layer.id" :value="String(layer.id)">
          {{ layer.label }}
        </option>
      </select>
    </div>

    <div class="form-item">
      <label>缓冲距离（米）</label>
      <input type="number" v-model.number="radius" min="1" class="custom-input" />
    </div>

    <button class="submit-btn" :disabled="submitting || !availableLayers.length" @click="run">
      {{ submitting ? "执行中..." : "执行分析" }}
    </button>
  </div>
</template>

<script setup lang="ts">
import { computed, ref, watch } from "vue";
import { ElMessage } from "element-plus";
import { submitAnalysisTask } from "@/api/analysis";
import emitter from "@/utils/bus";
import type { WorkbenchLayerItem } from "@/views/Workbench.vue";

const props = defineProps<{
  loadedLayers: WorkbenchLayerItem[];
}>();

const radius = ref(300);
const selectedLayerId = ref("");
const submitting = ref(false);

const availableLayers = computed(() =>
  props.loadedLayers.filter((layer) => layer.id !== 0 && !!layer.assetId),
);

watch(
  availableLayers,
  (layers) => {
    if (!layers.length) {
      selectedLayerId.value = "";
      return;
    }

    const hasSelectedLayer = layers.some(
      (layer) => String(layer.id) === selectedLayerId.value,
    );
    if (!hasSelectedLayer) {
      selectedLayerId.value = String(layers[0]!.id);
    }
  },
  { immediate: true },
);

const run = async () => {
  const selectedLayer = availableLayers.value.find(
    (layer) => String(layer.id) === selectedLayerId.value,
  );

  if (!selectedLayer?.assetId) {
    ElMessage.warning("请选择一个已发布的原始图层");
    return;
  }

  if (!Number.isFinite(radius.value) || radius.value <= 0) {
    ElMessage.warning("请输入大于 0 的缓冲距离");
    return;
  }

  submitting.value = true;
  try {
    const response = await submitAnalysisTask({
      toolId: 10004,
      assetId: selectedLayer.assetId,
      params: {
        radius: radius.value,
      },
    });

    emitter.emit("add-analysis-layer", {
      id: `analysis_${Date.now()}`,
      label: response.data.layerName || `${selectedLayer.label}_buffer`,
      type: "vector",
      visible: true,
      wmsUrl: response.data.wmsUrl,
      layers: response.data.layers,
      storeName: response.data.storeName || response.data.tempStoreName,
      resourceType: response.data.resourceType,
      cleanupGroup: response.data.cleanupGroup,
      geoJsonPath: response.data.geoJsonPath,
      sourceAssetId: selectedLayer.assetId,
    });

    ElMessage.success("缓冲区已生成并添加到地图");
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.message || error?.message || "缓冲分析失败");
  } finally {
    submitting.value = false;
  }
};
</script>

<style scoped>
.form-item { margin-bottom: 15px; }
.form-item label { display: block; font-size: 12px; margin-bottom: 5px; color: #94a3b8; }
.custom-input {
  width: 100%; background: #1e293b; border: 1px solid #334155;
  color: white; padding: 8px; border-radius: 4px;
}
.submit-btn {
  width: 100%; padding: 8px; background: #3b82f6; color: white;
  border: none; border-radius: 4px; cursor: pointer;
}
.submit-btn:disabled {
  cursor: not-allowed;
  opacity: 0.6;
}
</style>
