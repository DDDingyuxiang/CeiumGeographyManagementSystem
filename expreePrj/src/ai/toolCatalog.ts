...
export const GIS_TOOL_CATALOG: GisToolDefinition[] = [
  ...
  },
  {
    toolId: 20007,
    name: "植被指数(NDVI)",
    aliases: ["NDVI", "植被指数", "归一化植被指数", "vegetation index"],
    category: "raster",
    description: "基于红光波段和近红外波段计算 NDVI=(NIR-Red)/(NIR+Red)。",
    inputDataTypes: ["raster"],
    outputDataType: "raster",
    params: [
      {
      ...
      },
      {
        name: "nirBand",
        type: "number",
        description: "近红外波段序号，必须大于 0，且不能与 redBand 相同。",
        required: false,
        min: 0,
        defaultValue: 4,
      },
      {
        name: "nodata",
        type: "number",
        description: "输出 NoData 值，可省略，默认 -9999。",
        required: false,
        defaultValue: -9999,
      },
    ],
    planningRules: [
      "只能用于包含红光和近红外波段的 TIF/TIFF 栅格图层。",
      "用户没有说明波段号时，默认 redBand=3、nirBand=4，并在 warnings 中说明。",
      "如果用户明确给出传感器波段规则，按用户给定的红光和近红外波段填写。",
    ],
    examples: ["计算这幅影像的 NDVI", "用 4 波段近红外和 3 波段红光算植被指数"],
  },
  {
    toolId: 20008,
    name: "波段组合",
    aliases: ["波段合成", "波段组合", "真彩色", "假彩色", "band composite"],
    category: "raster",
    description: "将多波段 GeoTIFF 的指定三个波段合成为 RGB 影像。",
    inputDataTypes: ["raster"],
    outputDataType: "raster",
    params: [
      {
      ...
      },
      {
        name: "stretch",
        type: "string",
        description: "RGB 输出拉伸方式。",
        required: false,
        enumValues: ["none", "minmax", "percent"],
        defaultValue: "percent",
      },
      {
        name: "nodata",
        type: "number",
        description: "NoData 值，可省略。",
        required: false,
      },
    ],
    planningRules: [
      "只能用于多波段 TIF/TIFF 栅格图层。",
      "用户请求真彩色时，默认使用 redBand=3、greenBand=2、blueBand=1。",
      "用户请求标准假彩色时，默认使用 redBand=4、greenBand=3、blueBand=2。",
      "用户没有说明拉伸方式时，使用 stretch=percent。",
    ],
    examples: ["生成真彩色影像", "做 4/3/2 假彩色合成", "把 5、4、3 波段组合成 RGB"],
  },
  {
    toolId: 10003,
    name: "要素简化",
